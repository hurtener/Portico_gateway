...
V1.6 (Phase 13.5) is MCP Code Mode. Token-saving virtualised tool surface + Starlark sandbox. Opt-in per session; existing clients see no change. **Status: in progress** — shipped the JSON-Schema→`.pyi` stub translator, the hardened Starlark sandbox (`internal/mcp/codemode/runtime/`, threat-modelled in `docs/security/code-mode-threat-model.md`), the catalog projector, the session opt-in + four meta-tools, and `executeToolCode` with the governed in-sandbox dispatch path (acceptance #8 proven by integration test). Remaining: approval-suspend continuation, policy matchers, Console screens, CLI, and the RFC section. Concept docs: [`code-mode`](../concepts/code-mode.md), [`code-mode-savings`](../concepts/code-mode-savings.md), [`use-code-mode`](../how-to/use-code-mode.md).
