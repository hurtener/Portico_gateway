...
V2 Phase 14 (Agent Profiles) is the consumer-binding primitive — the V2 headline. **Status: done** — Migration 0020 (`agent_profiles` + four allowlist join tables + `agent_profile_jwt_bindings`, tenant-scoped, FK cascade); the `internal/profiles` resolver (60s/1024 LRU, fail-closed) + `internal/dataplane/middleware` step; full enforcement across MCP `tools/list`/`tools/call` (typed `agent_profile_violation` + audit), the prompt/resource/`skill://` surface and `skill://_index` body (acceptance #8), the LLM `/v1` alias gate, and scope intersection (#11, profile narrows never broadens); the `/api/agent-profiles` CRUD + JWT bindings + live-materialised `/surface` (#12); the `agent_profiles:` cold-start config block (idempotent seeding); profile matchers (`profile.id`/`name`/`includes_server`/`includes_alias`) + `require_profile_membership` in the policy DSL; the resolver P95 ≤ 1 ms gate (#14); `portico agents list|get|create|delete|bind|unbind|test` CLI; Console `/agents`. The synthesised default Profile keeps unbound principals on the full tenant surface (back-compat by construction). **§4.3 deviation:** snapshot-drift scoping (#9) is deferred — the background drift detector has no request context and `ActiveSession` carries no principal subject, so it cannot resolve a per-session Profile without a Phase 6 storage-projection change + a resolver dependency in a background component; drift is operator-facing observability while the agent-facing projection is already fully enforced, so a drift event for an out-of-profile server is at most a minor imprecision. The clean future fix (carry the subject on `ActiveSession`, inject the resolver, skip `snap.Servers` the Profile disallows) is recorded in the phase plan. Concept doc: [`agent-profiles`](../concepts/agent-profiles.md).

V2 Phase 15.5 (Semantic Cache + Virtual Keys + Hierarchical Budgets) is the Bifrost-shaped governance layer. **Status: done** — Migrations 0021/0022/0023 (governance customers/teams/virtual_keys + provider/model/mcp allowlists with the `profile_id` FK, budgets + ledger, SQLite cache_entries; all tenant-scoped, FK cascade). **Virtual Keys**: `pk-portico-<id>.<secret>` with `salt`+`HMAC-SHA256` (secret never stored), constant-time verify, the auth-boundary resolver (fail-closed, ambiguous `vk_unknown`/distinct `vk_revoked`), an auth-middleware branch that resolves a VK bearer before dev/JWT, REST CRUD (create/rotate/revoke, secret shown once) + `/budget` read, provider/model allowlist enforcement on `/v1/*` (`403 vk_scope_violation`), and MCP-server allowlist enforcement in the dispatcher (`-32007`, inherited by Code Mode). **Hierarchical budgets**: pure window math (calendar + rolling), the VK→Team→Customer→Tenant enforcer (pre-check most-specific-wins + atomic multi-level reconcile + debounced 80/95/100% warnings), wired into the LLM gateway (`429 budget_exceeded`), customers/teams/budgets REST, and the hierarchical `/budget` read API. **Semantic cache**: the §4.4 seam + key composition (tenant-first), `none`/`inmem`/`redis` drivers, gateway integration (hit-before-gating, store-after-dispatch), `Cache-Control` + `x-bf-cache-*` bypass, `/api/llm/cache/{config,stats,invalidate}`, and the LLM-gateway embedding generator (the semantic foundation). **Cross-cutting**: policy DSL `vk.*`/`cache.*`/`budget.*` matchers + `deny_on_cache_miss`/`force_cache_bypass`/`clamp_to_customer_budget` actions; Console `/governance/{customers,teams,virtual-keys,budgets,cache}` (secret-once modal, headroom bars, Playwright); the `portico governance` CLI; `scripts/smoke/phase-15.5.sh`. **§4.3 deviation:** the `weaviate` + `qdrant` semantic vector-store drivers (acceptance #2 semantic-hit, #10 weaviate leg) are a documented fast-follow — they need a live Weaviate/Qdrant to validate (mock-only would be false confidence) and the semantic gateway path must embed prompt text + pass vectors; the §4.4 cache seam + the shipped embedding seam make them a config-only addition, and exact-hash caching via the `redis` driver is the validated production path. Concept docs: [`semantic-cache`](../concepts/semantic-cache.md), [`virtual-keys`](../concepts/virtual-keys.md), [`hierarchical-budgets`](../concepts/hierarchical-budgets.md).

V2 Phase 16 (A2A) makes Portico fluent in a second agentic wire protocol on the same listener + envelope. **Status: core shipped** — Migrations 0024 (`a2a_peers`) + 0025 (`agent_profile_a2a_peers`/`_tasks`) + 0026 (`agent_profile_mcp_to_a2a_bridges`/`a2a_to_mcp_bridges`); the single-source `internal/a2a/protocol` wire types (`SpecVersion` 0.2.5, envelope, methods, errors, AgentCard, Task/Message/Part/Artifact); the southbound HTTP client + per-(tenant,peer) pool (vault-agnostic via an injected egress-auth factory); the governed `dispatch` service (`AllowsA2APeer` enforcement + `a2a.dispatch`/`agent_profile.violation` audit + `ErrProfileViolation` -32010); the northbound transport (`GET /a2a/.well-known/agent.json` + `POST /a2a`) mounted in the auth group (never bypasses the envelope) as a governed single-endpoint proxy (`metadata.portico_peer` routing); `/api/a2a/peers` CRUD + `refresh-card` ingestion (card cached + aggregated into Portico's card); Agent-Profile A2A allowlists + MCP→A2A bridge routes (data + domain + REST) with the **MCP→A2A bridge enforced** in the MCP dispatcher (`tools/call`→`message/send`, `AllowsA2ATask` gate, result translated back — acceptance #7); the `mocka2a` test peer; `scripts/smoke/phase-16.sh`; Console **A2A → Peers**. Acceptance #1/#2/#3/#5/#6/#7 + the governed envelope are proven (E2E `TestE2E_A2A_OutboundRoundtrip`, preflight 270 OK / 0 FAIL). **Fast-follows (documented):** the **A2A→MCP** bridge direction (acceptance #8 — needs a transient-session seam into the MCP dispatcher so an inbound A2A task invokes an MCP tool) and **scheduled** background card refresh + catalog `kind:a2a_task` drift (acceptance #9). Concept doc: [`a2a`](../concepts/a2a.md).
